...
2. **Configure Secrets**:
   Create a file named `secret.yaml` in this directory (`scripts/data_collector/cn_stock/secret.yaml`) to specify API credentials:
   ```yaml
...
```bash
python scripts/data_collector/cn_stock/collector.py download_data \
    --source akshare \
...
```bash
python scripts/data_collector/cn_stock/collector.py normalize_data \
    --source_dir ~/.qlib/cn_stock_data/source \
