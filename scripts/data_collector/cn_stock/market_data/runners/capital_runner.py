...
class CapitalRunner:
    ...
    def run(self, symbols: list, save_path: Path):
        def process_capital_symbol(sym):
            # Run all capital sub-tasks in parallel per symbol
            def fetch_margin():
                try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as inner_executor:
                futures = [
                    inner_executor.submit(fetch_margin),
                    inner_executor.submit(fetch_block),
                    inner_executor.submit(fetch_shareholders),
                    inner_executor.submit(fetch_dividends),
                    inner_executor.submit(fetch_fund_flow),
                ]
