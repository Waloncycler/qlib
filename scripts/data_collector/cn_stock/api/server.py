...
@app.get("/api/stock/{symbol}/intraday/{date}")
def get_intraday_data(symbol: str, date: str):
    ...
    try:
        ...
        # Return empty list gracefully instead of crashing frontend
        return {"status": "success", "data": []}
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Break on 4xx errors or if out of retries
                    break
            except Exception as e:
                if attempt < max_retries:
                    ...
                    raise HTTPException(status_code=500, detail=f"DeepSeek API Connection Error: {e}")
        
        if not response or response.status_code != 200:
            err_msg = response.text if response else "Unknown Error"
            raise HTTPException(status_code=500, detail=f"DeepSeek API Error: {err_msg}")
