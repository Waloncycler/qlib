...
ANALYST_PROMPT = """今天是 {date}。以下是系统过去 10 个交易日的综合数据。请作为数据分析师，客观提取关键事实。

{context}

# 你的任务：生成一份【事实摘要】（600 字以内，Markdown）

格式：
## 1. 情绪周期定位
（引用情绪打分的 10 日走势，定位当前处于"冰点/低迷/中性/活跃/亢奋"的哪个阶段，以及趋势方向）

## 2. 题材轮动全景
（哪些题材在这 10 天中反复出现？哪些是一日游？哪些在持续发酵？引用具体题材名和持续天数）

## 3. AI 早报信号复盘
（AI 早报的核心标的整体命中率如何？哪些题材推荐准确率高？哪些出现明显偏差？引用具体数值）

## 4. 核心个股观察
（列出值得关注的个股及其关键信息：涨停/跌停、成交量变化、公告事件等）

## 5. 量化异常信号
（PE/PB 分位、换手率分位、新高新低比、连板梯队变化等统计异常）
...
STRATEGIST_PROMPT = """今天是 {date}。以下是数据分析师提供的事实摘要：

{fact_summary}

# 你的任务：生成最终的【每日市场策略简报】

请先在内心进行多空博弈，然后输出以下内容：
...
### 博弈结论
（自我反驳后，给出平衡的方向判断：未来 1-3 天倾向看多/看空/震荡？）

## 二、情绪周期研判
（当前处于什么阶段？从情绪周期角度判断短期演变方向）

## 三、题材方向建议
（哪些题材值得关注？哪些应该回避？结合 AI 早报和题材轮动数据给出依据）

## 四、操作策略
- **仓位建议**: （根据情绪周期和博弈结论，给出具体百分比）
- **关注标的**: （从 AI 早报核心标的中筛选，结合题材持续性）
- **止损/风控**: （引用具体预警信号）

---
*数据来源: market_sentiment + AI早报 + 题材追踪 · 仅供研究参考*
...
def _build_sentiment_trend_10d(today: str) -> str:
    ...
    try:
        ...
        lines = ["# 近 10 日市场情绪趋势\n"]
        lines.append("| 日期 | 涨停 | 跌停 | 炸板 | 炸板率 | 情绪分 | 涨家 | 跌家 | 新高20 | 新低20 | 最高连板 | PE中位 | PB中位 | 换手中位 |")
        lines.append("|------|------|------|------|--------|--------|------|------|--------|--------|----------|--------|--------|----------|")

        for _, r in recent.iterrows():
            lines.append(
                f"| {r['date']} | {int(r.get('limit_up_count',0))} | {int(r.get('limit_down_count',0))} "
                f"| {int(r.get('broken_limit_up_count',0))} | {r.get('broken_limit_up_rate',0)*100:.0f}% "
                f"| {r.get('sentiment_score',0):.0f} | {int(r.get('up_count',0))} | {int(r.get('down_count',0))} "
                f"| {int(r.get('high20',0))} | {int(r.get('low20',0))} "
                f"| {int(r.get('max_lb_num',0))} "
                f"| {r.get('pe_median',0):.1f} | {r.get('pb_median',0):.2f} "
                f"| {r.get('turnover_median',0):.2f}% |"
            )
    ...
    try:
        ...
        for r in recent_reports:
            ...
            for g in r.get("stock_pool", []):
                ...
                if concept:
                    topic_dates[concept].append(date)
        ...
        if persistent:
            lines.append("**持续发酵题材（≥2天）:**")
            for topic, dates in sorted(persistent.items(), key=lambda x: -len(x[1])):
                lines.append(f"  - {topic} ({len(dates)}天: {', '.join(dates)})")

        if one_shot:
            lines.append(f"\n**一日游题材（{len(one_shot)}个）:** 一半可能只是市场试探，另一半可能是噪音。")

        # 最新一天的题材详情
        latest = recent_reports[-1]
        latest_date = latest.get("created_time", "")[:10]
        lines.append(f"\n**最新题材详情（{latest_date}）:**")
        for g in latest.get("stock_pool", []):
            concept = g.get("concept", "")
            is_new = g.get("is_new", False)
            tag = "🆕新" if is_new else "🔄续"
            core = [s["name"] for s in g.get("core_stocks", [])]
            lines.append(f"  - [{tag}] {concept}: 核心={', '.join(core[:5])}")
