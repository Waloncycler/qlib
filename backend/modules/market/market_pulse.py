...
格式：
## 1. 情绪周期定位
（引用情绪打分的 10 日走势和 30 日分位数，定位当前处于"冰点/低迷/中性/活跃/亢奋"的哪个阶段，以及趋势方向）

## 2. 宏观环境
（引用 CPI/PPI/PMI 数据，判断宏观对股市的影响方向。如无宏观数据则跳过此节）

## 3. 题材轮动全景
（哪些题材在这 10 天中反复出现？哪些是一日游？引用具体题材名、持续天数和催化事件，说明题材的核心逻辑）

## 4. AI 早报信号复盘
（AI 早报的全部标的整体命中率如何？核心标的 vs 其他标的命中率差异？哪些题材推荐准确率高？引用具体数值）

## 5. 核心个股观察
（列出全部核心标的及其他标的中 5 日涨幅居前的个股，结合其题材和 5 日 K 线走势）

## 6. 量化异常信号
（基于表中实际有数据的列，计算极端值或趋势异常。如新高/新低比、情绪分急升/急降、炸板率突变等。如果某列数据显示为"-"，说明无数据，跳过不提）
