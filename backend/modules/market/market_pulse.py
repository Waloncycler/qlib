...
ANALYST_PROMPT = """今天是 {date}。以下是系统过去 10 个交易日的综合数据。请作为数据分析师，逐个题材深入提取关键事实。

{context}

# 你的任务：生成一份【逐题材深度事实摘要】（1000 字以内，Markdown）

要求：不要笼统概括，必须逐个题材深入分析。每个题材都要引用具体的催化事件、个股表现和数据。
...
## 2. 宏观环境
（引用 CPI/PPI/PMI 具体数值，判断对市场的影响。如无数据则跳过）

## 3. 持续发酵题材逐个拆解（核心部分，必须每个都写）

对每个持续≥2天的题材，逐一分析：
- **题材名称**（持续X天）
  - 核心逻辑: （引用数据中的催化事件原文，不要模糊概括）
  - 涨价/事件催化剂: （具体数字，如"光纤预制棒涨价550%"、"六氟化钨暴涨200%"）
  - 代表个股: （引用5日K线具体涨跌幅，如"三孚股份5日+28.1%，06-26当日+8.8%"）
  - 产业链位置: （引用个股在题材中的三级细分归属和相关性，如"DDR细分→江波龙[互动易:2024存储营收174亿]"）

## 4. AI 早报信号准确度复盘
（逐日列出命中率，指出命中率最高和最低的日子。对比核心标的 vs 其他标的命中率差异。引用具体数值）

## 5. 一日游题材清单
（列出所有只出现1天的题材，简要标注）

## 6. 个股涨跌幅极值
（5日涨幅Top 3和跌幅Top 3，引用具体涨跌幅和所属题材）

## 7. 蓄势待发标的筛选
（引用"蓄势待发"数据，列出题材持续≥3天但个股5日涨幅±3%以内且未破趋势的标的。分析它们为何滞涨，以及补涨的概率）

# 约束
- 每个判断必须引用具体数值和具体催化事件原文
- 表格中显示为"-"的字段表示无数据，不要提及也不要猜测
- 禁止编造任何不在数据中的信息
- 禁止用"部分""一些""若干"等模糊词，必须写具体名称和数字
"""

STRATEGIST_PROMPT = """今天是 {date}。以下是数据分析师提供的逐题材事实摘要：

{fact_summary}

# 你的任务：生成最终的【每日市场策略简报】

请先在内心进行多空博弈，然后输出。要求每个论点必须引用分析师摘要中的具体题材名、催化事件和个股数据。

## 一、多空博弈

### 看多论点（引用具体题材和个股）
（必须写明：哪个题材？什么催化事件？哪只个股涨了多少？不要笼统说"部分题材有机会"）

### 看空论点（引用具体数据和指标）
（必须写明：哪个指标处于什么分位？哪些个股在跌？跌了多少？）

### 博弈结论
（基于上述具体数据，给出未来1-3天的方向判断）

## 二、情绪周期研判
（当前处于什么阶段？引用30日分位数的具体百分位。从情绪周期角度判断短期演变方向）

## 三、逐题材操作建议（核心部分，每个题材必须单独写）
...
- **判断**: 强化 / 分歧 / 退潮信号
- **依据**: （引用催化事件原文和个股涨跌数据）
- **操作**: 关注XX股（5日涨XX%），回避XX股（5日跌XX%）

（对每个持续≥3天的题材都写一节，不要跳过）

## 四、操作策略
- **仓位建议**: （具体百分比 + 调整条件）
- **重点关注标的**: （从核心标的中精选3-5只，每只写明题材归属+5日涨幅+推荐理由）
- **蓄势待发标的**: （引用分析师筛选的蓄势待发标的，分析补涨可能性）
- **明确回避标的**: （引用5日跌幅数据）
- **止损/风控**: （引用具体预警信号和阈值）
...
# 约束
- 每个观点都要引用事实摘要中的具体题材名、个股名和数值
- 禁止用"部分""一些""若干""等方向"等模糊表述
- 题材分析必须逐个写，不要合并笼统描述
- 1500 字以内
"""
