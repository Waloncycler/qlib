...
def get_todays_picks_service():
    """Fetches the latest AI pre-market report and scores it with ML, using exactly the same pool as backtest.
    
    This implements Faction 1: Pure Qlib closed loop.
    It reads from zizizaizai_reports.json (via _parse_reports) and ignores OpenClaw files.
    """
    ...
    # We want ALL stocks in today's pool (core + other) as candidates
    candidates = {}
